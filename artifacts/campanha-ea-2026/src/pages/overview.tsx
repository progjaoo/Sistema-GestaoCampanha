import { ArrowUpRight, Building2, MapPin, UsersRound, AlertTriangle, TrendingUp } from 'lucide-react';
import { Link } from 'wouter';
import { useGetCampaignOverview, useListCities, useListRegions } from '@workspace/api-client-react';
...
export default function OverviewPage() {
  ...
  if (overview.isLoading || regions.isLoading) return <OpsShell><PageHeading eyebrow="EA 2026 / panorama" title="Visão geral" description="A sala de operações da cobertura territorial." /><LoadingRows count={4} /></OpsShell>;
  if (overview.isError || regions.isError) return <OpsShell><PageHeading eyebrow="EA 2026 / panorama" title="Visão geral" description="A sala de operações da cobertura territorial." /><ErrorState onRetry={() => { void overview.refetch(); void regions.refetch(); }} /></OpsShell>;
  const data = overview.data;
  const regionItems = data?.regions ?? regions.data ?? [];
  const totals = data?.totals;
  const max = Math.max(...regionItems.map((item) => item.leadershipCount), 1);
  return <OpsShell>
    <PageHeading eyebrow="EA 2026 / panorama" title="Visão geral" description="Cobertura territorial, liderança local e sinais que pedem atenção." action={<Link href="/liderancas" className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-xs font-extrabold text-primary-foreground shadow-sm transition-transform hover:-translate-y-0.5" data-testid="link-explore-leaderships">Explorar lideranças <ArrowUpRight size={15} /></Link>} />
    ...
    <div className="grid gap-5 xl:grid-cols-[1.4fr_.8fr]">
      ...
      <section className="rounded-2xl bg-primary p-5 text-primary-foreground sm:p-7" data-testid="deputies-section">
        ...
        <div className="space-y-4">
          {(data?.topDeputies ?? []).slice(0, 6).map((deputy, i) => <div key={deputy.name} className="flex items-center gap-3 border-b border-primary-foreground/10 pb-4" data-testid={`deputy-row-${i}`}><div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary-foreground/10 text-xs font-bold">{String(i + 1).padStart(2, '0')}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{deputy.name}</p><p className="mt-0.5 text-[11px] text-primary-foreground/55">base de lideranças</p></div><span className="font-mono text-xs text-primary-foreground/75">{deputy.leadershipCount}</span></div>)}
          {!(data?.topDeputies?.length) && <p className="text-sm text-primary-foreground/65">Ainda não há dados de alianças.</p>}
        </div>
        <div className="mt-7 rounded-xl border border-primary-foreground/10 bg-primary-foreground/5 p-4"><div className="flex items-center justify-between"><span className="text-xs text-primary-foreground/60">Cidades carregadas</span><StatusPill tone="success">{cities.isLoading ? 'sincronizando' : `${cities.data?.length ?? 0} prontas`}</StatusPill></div></div>
      </section>
    </div>

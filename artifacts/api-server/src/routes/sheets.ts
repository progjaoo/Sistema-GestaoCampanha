...
import { CAMPAIGN_SPREADSHEET_ID, googleSheetsRequest } from "../lib/google-sheets";
...
router.get("/sheets/campaign", requirePermission("sheets:view"), async (req, res): Promise<void> => {
  const requestedTab = typeof req.query.tab === "string" ? req.query.tab.trim() : "";
  const metadataResponse = await googleSheetsRequest(
    `/v4/spreadsheets/${encodePathPart(CAMPAIGN_SPREADSHEET_ID)}?fields=properties.title,spreadsheetUrl,sheets.properties`,
  );
  if (!metadataResponse.ok) {
    res.status(502).json({ error: "Não foi possível consultar a planilha da campanha." });
    return;
  }
  ...
  const tabs = (metadata.sheets ?? [])
    ...
    .filter((sheet): sheet is { title: string; sheetId?: number; index?: number } => Boolean(sheet?.title));
  const tab = tabs.find((item) => item.title === requestedTab) ?? tabs[0];
  if (!tab) {
    res.json({ title: metadata.properties?.title ?? "Campanha EA 2026", spreadsheetUrl: metadata.spreadsheetUrl ?? null, tabs: [], selectedTab: null, headers: [], rows: [], matches: [] });
    return;
  }
  ...
  const matched = sourceRows.length
    ? await db.select({ id: leadershipsTable.id, sourceRow: leadershipsTable.sourceRow, name: leadershipsTable.name })
      .from(leadershipsTable)
      .where(and(eq(leadershipsTable.sourceSheet, tab.title), eq(leadershipsTable.needsReview, false)))
    : [];
  const matchedRows = new Set(matched.map((row) => row.sourceRow).filter((row): row is number => row !== null));
  ...
  res.json({
    ...
    headers,
    rows,
    matches: rows.filter((row) => matchedRows.has(row.sourceRow)).map((row) => row.sourceRow),
